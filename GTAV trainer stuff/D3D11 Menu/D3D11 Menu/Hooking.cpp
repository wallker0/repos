...
void main() {
	while (true) {
		...
		CheckKey(item[1], VK_F9);         //750K/s  $$	
		CheckKey(item[2], VK_NUMPAD5);    //1.5M    $$
		CheckKey(item[3], VK_F8);         //random  $$
		CheckKey(item[4], VK_F10);        //1*
		CheckKey(item[5], VK_F11);        //0*
		CheckKey(item[6], VK_MULTIPLY);   //quit

		CheckKey(item[7], VK_NUMPAD0);    //safe
		...
		if (item[7]) {
			...
			//1* (rp)
			if (item[4]) {
				PLAYER::REPORT_CRIME(player, 36, PLAYER::GET_WANTED_LEVEL_THRESHOLD(1));
...
static Hooking::NativeHandler _Handler(uint64_t origHash)
{
	...
	Log::Msg("getting and mapping nHashes");
	if (newHash == 0)
	{
		return nullptr;
	}
	...
	Log::Msg("getting and mapping nHashes2");
	...
	Log::Msg("getting and mapping nHashes3");
	return nullptr;
}
